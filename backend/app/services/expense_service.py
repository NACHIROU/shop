...
class ExpenseService:
    ...
    async def get_expense_by_id(expense_id: str, admin_id: str) -> ExpenseResponse:
        ...
        query = {
            "_id": ObjectId(expense_id) if ObjectId.is_valid(expense_id) else expense_id,
            "admin_id": admin_id
        }
        doc = await expenses_collection.find_one(query)
        if not doc:
    ...
    async def update_expense(expense_id: str, admin_id: str, update_data: ExpenseUpdate) -> ExpenseResponse:
        ...
        if update_dict:
            ...
            await expenses_collection.update_one(query, {"$set": update_dict})
    ...
    async def delete_expense(expense_id: str, admin_id: str):
        ...
        result = await expenses_collection.delete_one(query)
